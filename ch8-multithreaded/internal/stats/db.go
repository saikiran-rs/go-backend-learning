...
func (s *ScyllaStore) SaveEvents(ctx context.Context, events []Event) error {
	batch := s.session.Batch(gocql.UnloggedBatch)
	for _, e := range events {
		batch.Query(
			`INSERT INTO wiki.events (kafka_partition, kafka_offset, wiki_user, server_url, bot)
			 VALUES (?, ?, ?, ?, ?)`,
			e.Partition, e.Offset, e.User, e.ServerURL, e.Bot,
		)
	}
