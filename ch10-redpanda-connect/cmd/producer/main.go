...
import (
	"bufio"
	"ch10-redpanda-connect/internal/metrics"
	wikiv1 "ch10-redpanda-connect/internal/wiki/v1"
	"context"
	"encoding/json"
...
func produceStream(ctx context.Context, client *kgo.Client, url string) error {
	...
	for scanner.Scan() {
		...
		var rc recentChange
		if err := json.Unmarshal([]byte(rawData), &rc); err != nil {
			continue
		}
		metrics.StreamEventsConsumed.Inc()

		event := &wikiv1.WikiEvent{User: rc.User, Bot: rc.Bot, ServerUrl: rc.ServerURL}
		value, err := proto.Marshal(event)
		if err != nil {
			log.Printf("marshal error: %v", err)
			continue
		}
		record := &kgo.Record{Key: []byte(rc.ServerURL), Value: value}
		client.Produce(ctx, record, func(_ *kgo.Record, err error) {
			if err != nil {
				log.Printf("produce error: %v", err)
				return
			}
			metrics.RedpandaEventsProduced.Inc()
		})
	}
	return scanner.Err()
}
